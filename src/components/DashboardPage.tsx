...
function Dashboard() {
  ...
  const handleAdminLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const isPrimaryAdmin =
        formattedEmail === "felixjonathan201@gmail.com" ||
        formattedEmail === "devhaitian@gmail.com";

      if (snap.empty) {
        if (isPrimaryAdmin) {
          console.log("[Debug] Seeding primary admin doc into /admins collection.");
          const newAdminData = {
            email: formattedEmail,
            password: password.trim(),
            nom: formattedEmail === "felixjonathan201@gmail.com" ? "Félix Jonathan" : "Haitian Dev",
            role: "admin",
          };
          await addDoc(collection(db, "admins"), newAdminData);
          adminDoc = newAdminData;
        } else {
        ...
        if (adminDoc.password !== password.trim()) {
          if (isPrimaryAdmin) {
            console.log("[Debug] Syncing primary admin password to the newly entered password.");
            const docRef = snap.docs[0].ref;
            await updateDoc(docRef, { password: password.trim() });
            adminDoc.password = password.trim();
          } else {
  ...
  const [activeTab, setActiveTab] = useState<
    "users" | "notifications" | "settings" | "messages" | "gallery"
  >("users");
  ...
  const [galleryMsg, setGalleryMsg] = useState<{ type: "success" | "error"; text: string } | null>(
    null,
  );
  const [gallerySortOrder, setGallerySortOrder] = useState<"recent" | "oldest">("recent");
  ...
  // Create / add a new gallery photo item
  const handleCreateGalleryPhoto = async (e: React.FormEvent) => {
    e.preventDefault();
    setGalleryMsg(null);
    if (!photoTitle.trim()) {
      setGalleryMsg({ type: "error", text: "Veuillez spécifier un titre pour la photo." });
      return;
    }

    if (!uploadFile && !photoUrl.trim()) {
      setGalleryMsg({
        type: "error",
        text: "Veuillez sélectionner un fichier image à uploader ou fournir une URL d'image valide.",
      });
      return;
    }
    ...
    try {
      ...
      // If there is an upload file, process uploading with Storage
      if (uploadFile) {
        setGalleryMsg({ type: "success", text: "Téléversement de l'image en cours..." });
        finalImageUrl = await uploadGalleryImage(uploadFile);
      }

      const activeUid = user?.uid || "admin";
      const activeEmail = user?.email || "admin@example.com";

      await addGalleryPhoto(activeUid, activeEmail, {
        title: photoTitle.trim(),
        description: photoDescription.trim(),
        imageUrl: finalImageUrl,
      });

      setGalleryMsg({ type: "success", text: "Photo intégrée et publiée avec succès !" });
      ...
      setGalleryMsg({
        type: "error",
        text: `Une erreur est survenue lors de l'intégration : ${errorVal.message || String(err)}`,
      });
    } finally {
  ...
  // Trigger inline editing for a photo
  const startEditingPhoto = (photo: GalleryPhoto) => {
    setEditingPhotoId(photo.id || null);
    setEditPhotoTitle(photo.title);
    setEditPhotoDescription(photo.description || "");
  };

  // Save modified photo details
  const handleUpdateGalleryPhotoDetails = async (e: React.FormEvent) => {
    e.preventDefault();
    setGalleryMsg(null);
    if (!editingPhotoId) return;

    if (!editPhotoTitle.trim()) {
      setGalleryMsg({ type: "error", text: "Le titre de la photo ne peut pas être vide." });
      return;
    }
    ...
    try {
      ...
      await updateGalleryPhoto(activeUid, activeEmail, editingPhotoId, {
        title: editPhotoTitle.trim(),
        description: editPhotoDescription.trim(),
      });

      setGalleryMsg({ type: "success", text: "Photo modifiée avec succès !" });
      ...
      setGalleryMsg({
        type: "error",
        text: `Erreur de modification : ${errorVal.message || String(err)}`,
      });
    } finally {
      setGalleryLoading(false);
    }
  ...
  const handleDeleteGalleryPhotoItem = async (photoId: string) => {
    if (
      !window.confirm(
        "Êtes-vous sûr de vouloir supprimer cette photo définitivement de la galerie ?",
      )
    ) {
      return;
    }
    ...
    try {
      ...
      await deleteGalleryPhoto(activeUid, activeEmail, photoId);
      setGalleryMsg({ type: "success", text: "La photo a été retirée de la galerie." });
    } catch (err: unknown) {
      console.error("Error deleting photo:", err);
      const errorVal = err as Error;
      setGalleryMsg({
        type: "error",
        text: `Erreur de suppression : ${errorVal.message || String(err)}`,
      });
    } finally {
      setGalleryLoading(false);
    }
  ...
  useEffect(() => {
    ...
    // 4.5 Subscribe to Gallery Photos
    ...
    const unsubscribeGallery = onSnapshot(
      ...
      (snap) => {
        ...
        snap.forEach((docSnap) => {
          photoList.push({ id: docSnap.id, ...docSnap.data() } as GalleryPhoto);
        });
        setGalleryList(photoList);
      },
      (error) => {
        console.error("Gallery list listener failed:", error);
      },
    );
  ...
  const handleDeleteNotification = async (notificationId: string) => {
    ...
    const confirmDelete = window.confirm(
      "Êtes-vous sûr de vouloir supprimer cette notification ? Cette action est définitive.",
    );
    if (!confirmDelete) return;

    setIsDeletingNotif(notificationId);
    setNotifSuccess("");
    setNotifError("");

    try {
      if (user && user.uid === "demo-admin-uid-bypass") {
        setNotifications((prev) => prev.filter((n) => n.id !== notificationId));
        setNotifSuccess("Notification supprimée avec succès (Simulation) !");
        return;
      }

      if (!user) return;
      await deleteNotification(user.uid, user.email || "", notificationId);
      setNotifSuccess("Notification supprimée avec succès !");
    } catch (err) {
      console.error("Error deleting notification:", err);
      setNotifError("Une erreur est survenue lors de la suppression de la notification.");
    } finally {
  ...
  return (
    <div className="min-h-screen bg-background px-6 py-10 text-foreground">
      <div className="mx-auto max-w-7xl">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {(!n.createdBy || n.createdBy === user?.uid) && (
                            <button
                              ...
                              disabled={isDeletingNotif === n.id}
                              className="text-red-400 hover:text-red-300 text-[10px] font-semibold flex items-center gap-1 transition-colors pl-2 border-l border-[var(--card-border)]/50 cursor-pointer disabled:opacity-50"
                              title="Supprimer cette notification"
                            >
                              {isDeletingNotif === n.id ? "..." : "🗑️ Supprimer"}
                            </button>
                          )}
                        </div>
                      </div>
        ...
        {/* 5. GESTION DE LA GALERIE TAB */}
        {activeTab === "gallery" && (
          <div className="mt-8 space-y-8 animate-fadeIn text-left animate-in fade-in duration-200">
            {/* Header section */}
            <div className="border-b border-[var(--card-border)]/40 pb-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <h3 className="font-display text-xl text-[var(--accent-gold)] flex items-center gap-2">
                  <span>🖼️</span> Gestion de la Galerie Photos
                </h3>
                <p className="text-xs text-muted-foreground mt-1 font-sans">
                  Ajoutez, modifiez ou retirez les photos présentées dans la Galerie publique du
                  site ALPHA Préfac.
                </p>
              </div>
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  onChange={(e) => setGallerySortOrder(e.target.value as "recent" | "oldest")}
                  className="rounded-lg border border-[var(--card-border)] bg-card px-3 py-1.5 text-xs text-foreground font-medium focus:outline-none"
                >
                  <option value="recent">Récent en premier</option>
                  <option value="oldest">Ancien en premier</option>
                </select>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              {/* Form panel section (Left side: 1 col) */}
              <div className="lg:col-span-1 rounded-2xl border border-[var(--card-border)] bg-card/40 p-6 space-y-6 h-fit">
                <h4 className="font-semibold text-sm text-[var(--accent-gold)] uppercase font-mono tracking-wider">
                ...
                <form onSubmit={handleCreateGalleryPhoto} className="space-y-4">
                  {/* Photo Title */}
                  <div>
                    <label className="text-[10px] block font-mono text-muted-foreground uppercase mb-1 font-bold">
                      Titre de la photo :
                    </label>
                    <input
                      type="text"
                      placeholder="Ex: Séance de Physique cohorte médecine"
                      value={photoTitle}
                      onChange={(e) => setPhotoTitle(e.target.value)}
                      className="w-full text-xs rounded-lg border border-[var(--card-border)] bg-card/60 px-3 py-2.5 text-foreground focus:outline-none focus:ring-1 focus:ring-[var(--accent-gold)]/50"
                  ...
                  <div>
                    ...
                    <textarea
                      placeholder="Fournissez une brève explication sur l'événement ou le contexte..."
                      rows={3}
                      value={photoDescription}
                      onChange={(e) => setPhotoDescription(e.target.value)}
                      className="w-full text-xs rounded-lg border border-[var(--card-border)] bg-card/60 px-3 py-2.5 text-foreground focus:outline-none focus:ring-1 focus:ring-[var(--accent-gold)]/50 resize-none"
                      disabled={galleryLoading}
                    />
                  </div>

                  {/* Drag-and-drop / select File Uploader */}
                  <div>
                    <label className="text-[10px] block font-mono text-muted-foreground uppercase mb-2 font-bold">
                      Sélectionner l'image (Firebase Storage) :
                    </label>
                    <div
                      className="border-2 border-dashed border-[var(--card-border)] hover:border-[var(--accent-gold)]/50 rounded-xl p-4 text-center cursor-pointer transition-all bg-card/25"
                      ...
                      {uploadFile ? (
                        <div className="space-y-1">
                          ...
                          <p className="text-[10px] text-muted-foreground font-mono">
                            {(uploadFile.size / 1024 / 1024).toFixed(2)} MB · Cliquez pour remplacer
                          </p>
                        </div>
                      ) : (
                        <div className="space-y-1 py-1">
                          ...
                          <p className="text-xs font-medium text-foreground">
                            Glissez ou cliquez pour charger
                          </p>
                          <p className="text-[9px] text-muted-foreground">
                            Fichiers JPEG, PNG, WEBP acceptés
                          </p>
                        </div>
                      )}
                    </div>
                  ...
                  <div className="pt-2 border-t border-[var(--card-border)]/40">
                    <p className="text-[9px] text-muted-foreground mb-2 text-center">
                      — OU Spécifier une adresse URL d'image existante —
                    </p>
                    ...
                    <input
                      ...
                      className="w-full text-[11px] rounded-lg border border-[var(--card-border)] bg-card/60 px-3 py-2 text-foreground focus:outline-none focus:ring-1 focus:ring-[var(--accent-gold)]/50"
                      disabled={galleryLoading}
                    />
                  </div>

                  {/* Live Thumbnail publication preview */}
                  {(uploadFile || photoUrl.trim()) && (
                    <div className="pt-2">
                      <span className="text-[10px] block font-mono text-muted-foreground uppercase mb-1.5 font-bold">
                        Aperçu avant publication :
                      </span>
                      <div className="aspect-video w-full rounded-xl overflow-hidden border border-[var(--card-border)] bg-black/40 relative">
                        <img
                          src={uploadFile ? URL.createObjectURL(uploadFile) : photoUrl.trim()}
                          alt="Prepublication preview"
                  ...
                  <button
                    type="submit"
                    disabled={galleryLoading}
                    className="w-full rounded-lg bg-[var(--accent-gold)] py-2.5 text-xs font-bold text-[var(--primary-foreground)] shadow-[0_8px_24px_-8px_rgba(234,200,80,0.45)] hover:-translate-y-0.5 transition-transform disabled:opacity-50 cursor-pointer text-center"
                  >
                    {galleryLoading ? "Publication en cours..." : "✓ Publier sur la Galerie"}
                  </button>
                </form>
              </div>

              {/* Photos List Grid (Right side: 2 cols) */}
              <div className="lg:col-span-2 space-y-4">
                <h4 className="font-semibold text-sm text-[var(--accent-gold)] uppercase font-mono tracking-wider">
                  ✧ Liste des Photos publiées ({galleryList.length})
                </h4>

                {galleryList.length === 0 ? (
                  <div className="rounded-2xl border border-[var(--card-border)] bg-card/10 p-12 text-center text-muted-foreground animate-pulse">
                    <p className="font-medium text-sm">
                      Aucune photo publiée pour l'instant dans la Galerie.
                    </p>
                    <p className="text-[11px] mt-1">
                      Utilisez l'onglet de gauche pour intégrer votre premier élément.
                    </p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {/* Sort applied gallery list copy */}
                    {[...galleryList]
                      .sort((a, b) => {
                        ...
                        return gallerySortOrder === "recent" ? timeB - timeA : timeA - timeB;
                      })
                      .map((photo) => (
                        <div
                          key={photo.id}
                          className="rounded-xl border border-[var(--card-border)] bg-card/30 p-4 transition-all hover:bg-card/40 flex flex-col justify-between"
                        >
                          <div className="space-y-3">
                            <div className="aspect-video w-full rounded-lg overflow-hidden border border-[var(--card-border)] bg-black/20">
                              <img
                                src={photo.imageUrl}
                                alt={photo.title}
                            ...
                            <div>
                              <h5 className="font-semibold text-sm text-foreground tracking-tight line-clamp-1">
                                {photo.title}
                              </h5>
                              <p className="text-xs text-muted-foreground mt-1 min-h-[32px] line-clamp-2 leading-relaxed">
                                {photo.description ||
                                  "Aucune description fournie pour cette photo."}
                              </p>
                            </div>
                          </div>

                          <div className="pt-3 border-t border-[var(--card-border)]/40 mt-3 flex items-center justify-between text-[10px] font-mono text-muted-foreground">
                            <span>
                              {photo.createdAt ? formatDate(photo.createdAt) : "Date inconnue"}
                            </span>
                            <div className="flex items-center gap-1.5">
                              <button
                                onClick={() => startEditingPhoto(photo)}
                                className="px-2 py-1 rounded bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 font-semibold cursor-pointer"
                                title="Modifier"
                              >
                                ✏️ Modifier
                              </button>
                              <button
                                onClick={() => photo.id && handleDeleteGalleryPhotoItem(photo.id)}
                                className="px-2 py-1 rounded bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 font-semibold cursor-pointer"
                                title="Retirer"
                              >
                                🗑️ Retirer
                              </button>
            ...
            {/* Inline Editing Photo Overlay Modal */}
            {editingPhotoId && (
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
                <div className="mx-auto max-w-md w-full bg-zinc-900 border border-zinc-800 rounded-2xl p-6 shadow-2xl relative space-y-4">
                  <h4 className="font-bold text-base text-[var(--accent-gold)] flex items-center gap-2">
                    <span>✏️</span> Modifier les détails de la Photo
                  </h4>
                  <hr className="border-zinc-800" />

                  <form onSubmit={handleUpdateGalleryPhotoDetails} className="space-y-4">
                    <div>
                      <label className="text-[10px] block font-mono text-zinc-400 uppercase mb-1 font-bold">
                        Titre de la photo :
                      </label>
                      <input
                        type="text"
                        ...
                        className="w-full text-xs rounded-lg border border-zinc-800 bg-zinc-950 px-3 py-2 text-white focus:outline-none focus:ring-1 focus:ring-[var(--accent-gold)]/50"
                        title="Titre"
                        required
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        rows={4}
                        className="w-full text-xs rounded-lg border border-zinc-800 bg-zinc-950 px-3 py-2 text-white focus:outline-none focus:ring-1 focus:ring-[var(--accent-gold)]/50 resize-none"
                        title="Description"
                      />
                    </div>
                    ...
                    <div className="flex gap-3 pt-2">
                      <button
                        type="submit"
                        disabled={galleryLoading}
                        className="flex-grow rounded-lg bg-[var(--accent-gold)] py-2 text-xs font-bold text-[var(--primary-foreground)] hover:opacity-90 disabled:opacity-50 cursor-pointer"
                      >
                        Enregistrer
                      </button>
                      <button
                        ...
                        className="rounded-lg border border-zinc-800 px-4 py-2 text-xs font-medium text-zinc-400 hover:text-white cursor-pointer"
                      >
                        Annuler
                      </button>
                    </div>
